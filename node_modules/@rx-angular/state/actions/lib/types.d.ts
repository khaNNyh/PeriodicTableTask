...
type FunctionParamsOrValueType<U, K, F> = InferArguments<Select<U, K>> extends never ? [F] : InferArguments<Select<U, K>>;
export type Actions = {};
export type SubjectMap<T> = {
    [K in keyof T]: Subject<T[K]>;
};
export type EffectMap<T> = {
    [K in keyof T]: Subscription;
};
export type ActionTransforms<T extends {}> = Partial<{
    [K in keyof T]: (...args: any[]) => T[K];
}>;
export type ActionDispatchFn<O extends unknown[]> = (...value: InstanceOrType<O>) => void;
...
export type ActionEffects<T extends Actions, O = T> = {
    [K in ExtractString<T> as `on${Capitalize<string & K>}`]: <R>(fn: OperatorFunction<T[K], R>, sideEffectFn?: (value: R) => void) => () => void;
};
export type RxActions<T extends Actions, U extends {} = T> = ActionDispatchers<T, U> & ActionObservables<T> & ActionEffects<T> & ((slice: Partial<T>) => void) & {
